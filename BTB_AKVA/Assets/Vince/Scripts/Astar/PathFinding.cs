...
public class PathFinding : MonoBehaviour
{
    ...
    IEnumerator FindPath(Vector3 startPos, Vector3 targetPos)
    {
        ...
        {
            Heap<NodeScript> openSet = new Heap<NodeScript>(grid.MaxSize);
            HashSet<NodeScript> closedSet = new HashSet<NodeScript>();
            openSet.Add(startNode);

            while (openSet.Count > 0)
            {
                NodeScript currentNode = openSet.RemoveFirst();
    ...
    Vector3[] RetracePath(NodeScript startNode, NodeScript endNode)
    {
