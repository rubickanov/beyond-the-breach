...
namespace AKVA.Assets.Vince.Scripts.SceneManager
{
    public class Room3State : SceneState
    {
        ...
        void AITask(SceneStateManager state)
        {
            if (aiActive)
            {
                if (Vector3.Distance(state.listOfAI.items[0].transform.position, state.aiDestination[0].position) < 2f && !task[0] && !aiEnabled)
                {
                ...
                else if (task[1] && !task[2] && Vector3.Distance(state.listOfAI.items[0].transform.position, state.listOfAI.items[0].GetComponent<AIStateManager>().pathPoints[12].position) < 1f)
                {
                    task[2] = true;
                ...
                else if (task[3] && !task[4] && Vector3.Distance(state.listOfAI.items[1].transform.position, state.listOfAI.items[1].GetComponent<AIStateManager>().pathPoints[11].position) < 1f)
                {
                    task[4] = true;
                    state.imagesAppeared[5].value = true; // correct
                    state.StartCoroutine(StartAITask(state, 1, 4f, true)); // go back to pos
                }
                else if (task[4] && !task[5] && Vector3.Distance(state.listOfAI.items[1].transform.position, state.listOfAI.items[1].GetComponent<AIStateManager>().pathPoints[12].position) < 1f)
                {
                    task[5] = true;
                ...
                {
                    task[6] = true;
                    state.StartCoroutine(showImageDelay(state, 2, 7f)); // correct
                    state.StartCoroutine(showImageDelay(state, 2, 9f)); // show image of switch
                    state.StartCoroutine(StartAITask(state, 2, 11f, true)); // go to switch pos
                }else if (task[6] && !task[7] && Vector3.Distance(state.listOfAI.items[2].transform.position, state.listOfAI.items[2].GetComponent<AIStateManager>().pathPoints[11].position) < 1f)
                {
                    task[7] = true;
